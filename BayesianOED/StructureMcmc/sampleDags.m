function [samples, diagnostics, runningSum] = sampleDags(logPriorFn, allFamilyLogMargLik, trueDag, varargin)
...
hammingDist = zeros(1, nSamples); %added MZ 10/25/18
...
for si=1:nSamples
    ...
    if jumpTypeRand>jumpFracs(1)
        ...
        if isReachabilityStale
            AB = reachability_graph(dag'); % AB is the ancestor matrix for dag (uses expm function to calculate)
            isReachabilityStale = false;
        end
        
        if areNeighboursStale && jumpTypeRand>jumpFracs(2)
            [nbrs, ops, nodes, A] = mk_nbrs_of_digraph(dag, AB); %Make all digraphs that differ from dag by a single edge deletion, addition or reversal
	...
	if jumpTypeRand<jumpFracs(1)
        ...
        % calculate log marginal likelihood for proposed dag
		[dagp logQp] = sampleDagFromEdgeMarginals( edgeMarginals );
		logMargLikp = logMargLikDag( dagp, allFamilyLogMargLik);
		
        % calculate log marginal likelihood for current state dag
...
%                 dag = dagp;
    ...
    hammingDist(si) = nnz(xor(dag,trueDag)); 
	if si > burnin && mod(si-burnin-1, thinning)==0
        nActualSamples = nActualSamples + 1; 
		
		dagKey = dag2char(dag); % dagKey is the representation of the dag adj matrix as a character
		dagValue = samples.HT.get(dagKey);
		if isempty(dagValue)
			count = 1;
			ind = orderIndex; % if dag has never been sampled before, ind will store the MCMC iteration for when it was first sampled
			orderIndex = orderIndex + 1;
			
			samples.order2DagHT.put( ind, dagKey ); %order2DagHT stores the MCMC iteration for when the dag represented by dagKey was first sampled
		else
			count = dagValue(1) + 1; %if dag already sampled previously, increment the count of times it has been visited
			ind = dagValue(2); % this doesn't change, it's the iteration when dag was first sampled (?)
		end
		samples.order(nActualSamples) = ind;% so samples.order would keep track of the iteration for when a new dag was sampled
...
samples.hammingDist = hammingDist;
