function [marginals map] = computeAllPathProb_Exact( logPrior, allFamilyLogMargLik )
...
for gi=1:length(dags)
    ...
    marginals = marginals + posterior(gi) * reachability_graph(char2dag(char(dags(gi,:)), nNodes));
