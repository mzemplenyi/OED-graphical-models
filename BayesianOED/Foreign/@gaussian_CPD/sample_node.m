...
%  y = gsamp(CPD.mean(:,i), CPD.cov(:,:,i), 1);
   y = normrnd(CPD.mean(:,i), CPD.cov(:,:,i), 1); % MZ added 11/1/2018 instead of gsamp()
else
  pev = pev(:);
  x = cat(1, pev{CPD.cps});
  % y = gsamp(CPD.mean(:,i) + CPD.weights(:,:,i)*x(:), CPD.cov(:,:,i), 1);
  y = normrnd(CPD.mean(:,i) + CPD.weights(:,:,i)*x(:), CPD.cov(:,:,i), 1); % MZ added 11/1/2018 instead of gsamp()
end
y = y(:);
